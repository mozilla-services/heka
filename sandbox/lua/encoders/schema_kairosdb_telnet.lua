...
--[=[
Converts full Heka message contents to line protocol for KairsDB telnet API.
Optionally includes all standard message fields
...
request sent to the KairosDB write API, using the `tag_fields` config option.
All dynamic fields in the Heka message are converted to separate points
separated by newlines that are submitted to KairosDB.

.. note::
    This encoder is intended for use with KairosDB.
...
- timestamp_precision (string, optional, default "ms")
    ...
    values supported by the KairosDB write API (ms, s).
...
.. code-block:: ini
    ...
    [KairosDBTelnetEncoder]
    type = "SandboxEncoder"
    filename = "lua_encoders/schema_kairosdb_telnet.lua"

        [KairosDBTelnetEncoder.config]
    ...
    address = "kairosdbserver.example.com:4242"

*Example Output*

.. code-block:: none

    put 5MinAvg 1434932024 0.110000 Hostname=myhost Environment=dev
    put 1MinAvg 1434932024 0.110000 Hostname=myhost Environment=dev
    put 15MinAvg 1434932024 0.170000 Hostname=myhost Environment=dev
...
function process_message()
    local api_message = ts_line_protocol.kairosdb_telnet_msg(config)
    ...
    inject_payload("txt", "kairosdb_telnet", table.concat(api_message, "\n").."\n")
