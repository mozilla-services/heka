...
Parses a payload containing the contents of file `/proc/stat`. This also
...
:Fields:
    ...
    | name:"TickerInterval" type:double value:1
    ...
    | name:"index_count" type:number value:10
    | name:"index" type:string value:[cpu,cpu#,TickerInterval,ctxt,btime...]
...
function process_message()
    ...
    -- var index is a list of all fields found in the payload
    local count = 1
    local index = {}
    for k in pairs(msg.Fields) do
        index[count] = k
        count = count + 1
    end
    msg.Fields.index = index
    msg.Fields.index_count = count
