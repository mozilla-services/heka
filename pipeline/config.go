...
func NewPipelineConfig(globals *GlobalConfigStruct) (config *PipelineConfig) {
	...
	config.cfgMsgs = make([]string, 0, 4)
...
// Send logging through here so we can easily change logging mechanisms in the
// future.
func (self *PipelineConfig) log(msg string, v ...interface{}) {
	if len(v) > 0 {
...
// Store config log messages for tests.
func (self *PipelineConfig) logCfg(msg string, v ...interface{}) {
	if len(v) > 0 {
		msg = fmt.Sprintf(msg, v...)
	}
	self.cfgMsgs = append(self.cfgMsgs, msg)
	self.log(msg)
}
...
func (self *PipelineConfig) loadSection(sectionName string,
	...
	if err = toml.PrimitiveDecode(configSection, &pluginGlobals); err != nil {
		self.logCfg("Unable to decode config for plugin: %s, error: %s", wrapper.name,
			err.Error())
		errcnt++
		return
	}
	...
	if wrapper.pluginCreator, ok = AvailablePlugins[pluginType]; !ok {
		self.logCfg("No such plugin: %s", wrapper.name)
		errcnt++
		return
	}
	...
	if config, err = LoadConfigStruct(configSection, plugin); err != nil {
		self.logCfg("Can't load config for %s '%s': %s", sectionName, wrapper.name, err)
		errcnt++
		return
	}
	...
	if err = configPlugin(); err != nil {
		self.logCfg("Initialization failed for '%s': %s", sectionName, err)
		errcnt++
		return
	}
	...
	if len(pluginCats) < 2 {
		self.logCfg("Type doesn't contain valid plugin name: %s", pluginType)
		errcnt++
		return
	}
	...
	if pluginCategory == "Decoder" {
		if pluginGlobals.Encoding != "" {
			...
			if err != nil {
				self.logCfg("Can't register decoder '%s' for encoding '%s': %s",
	...
	if pluginGlobals.Matcher != "" {
		if matcher, err = NewMatchRunner(pluginGlobals.Matcher,
			...
			self.logCfg("Can't create message matcher for '%s': %s",
				wrapper.name, err)
			errcnt++
			return
		}
