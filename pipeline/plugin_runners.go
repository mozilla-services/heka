...
func NewInputRunner(name string, input Input, config CommonInputConfig) (ir InputRunner) {
	...
	if config.PrintDecodeFailures != nil {
		runner.printDecodeFailures = *config.PrintDecodeFailures
	}
...
func (ir *iRunner) getDeliverFunc(token string) (DeliverFunc, DecoderRunner, Decoder) {
	...
	if !ir.syncDecode {
		...
		dr.SetFailureHandling(ir.printDecodeFailures, ir.sendDecodeFailures)
		inChan := dr.InChan()
		deliver = func(pack *PipelinePack) {
			inChan <- pack
		}
