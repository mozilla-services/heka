...
func findMessage(buf []byte, header *Header, message *[]byte) (pos int, ok bool) {
	...
	if pos != -1 {
		if len(buf)-pos > 1 {
			...
			headerEnd := pos + headerLength + HEADER_FRAMING_SIZE
			if len(buf) >= headerEnd {
				if header.MessageLength != nil || decodeHeader(buf[pos+HEADER_DELIMITER_SIZE:headerEnd], header) {
					messageEnd := headerEnd + int(header.GetMessageLength())
					if len(buf) >= messageEnd {
						requiredSize := messageEnd - headerEnd // max size already tested in decode header
						if cap(*message) < requiredSize {
							*message = make([]byte, requiredSize)
						} else {
							*message = (*message)[:requiredSize]
						}
...
func (u *UdpInput) Init(config interface{}) (err error) {
	...
	if u.config.ParserType == "message.proto" {
		...
		u.config.Decoder = "ProtobufDecoder"
...
func (t *TcpInput) Init(config interface{}) error {
	...
	if t.config.ParserType == "message.proto" {
		...
		t.config.Decoder = "ProtobufDecoder"
