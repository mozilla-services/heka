...
%token VAR_FIELDS
%token STRING_VALUE NUMERIC_VALUE REGEXP_VALUE NIL_VALUE DATE_VALUE DATETIME_VALUE
%token TRUE FALSE
...
string_test : string_vars relational STRING_VALUE
   ...
   |   VAR_TIMESTAMP relational STRING_VALUE
       {
       //fmt.Println("timestamp_string_test regexp", $1, $2, $3)
       timestamp, tokenId, err := parseFullDateTime($3.token)

       if err != nil {
            log.Printf("Invalid datetime value for Timestamp field: %s", err)
            return 1
       }
       $3.tokenId = tokenId
       $3.double = float64(timestamp.UnixNano())
       nodes = append(nodes, &tree{stmt:&Statement{$1, $2, $3}})
       }
;
numeric_test : numeric_vars relational NUMERIC_VALUE
   ...
   |    VAR_TIMESTAMP relational NUMERIC_VALUE
        {
        //fmt.Println("numeric_test", $1, $2, $3)
        nodes = append(nodes, &tree{stmt:&Statement{$1, $2, $3}})
        }
