...
func IrcOutputSpec(c gs.Context) {
	...
	c.Specify("An IrcOutput", func() {
		...
		c.Specify("that is started", func() {
			...
			c.Specify("joins before sending a message, and parts after, when configured to", func() {
				config.JoinAndPart = true
				err := ircOutput.Init(config)
				c.Assume(err, gs.IsNil)
				...
				channelJoinStatus := &ircOutput.JoinedChannels[0]

				// verify we aren't in any irc channels yet
				c.Expect(*channelJoinStatus, gs.Equals, NOTJOINED)
				...
				p := <-ircOutput.OutQueue
				ircOutput.OutQueue <- p
				// verify we've joined the irc channels
				c.Expect(*channelJoinStatus, gs.Equals, JOINED)

				// once it's arrived send a tick so it gets processed
				tickChan <- time.Now()

				// Wait for the message to get delivered before we check to see if
				// have left the irc channels
				<-mockIrcConn.delivered

				// verify we aren't in any irc channels after sending the message.
				c.Expect(*channelJoinStatus, gs.Equals, NOTJOINED)
			...
			c.Specify("sends backlogged messages", func() {
				config.QueueSize = 1
				err := ircOutput.Init(config)
				c.Assume(err, gs.IsNil)
				...
				// Leave the irc channel to fill up the backlog queue
				...
				// The backlog queue should have 1 message in it
				queue := ircOutput.BacklogQueues[0]
				// Wait for the message to avoid a race, and then put it back
				p = <-queue
				queue <- p
				c.Expect(len(queue), gs.Equals, 1)

				// Join the irc channels to enable sending the message
				ircOutput.Conn.Join(ircChan)
				tickChan <- time.Now()
