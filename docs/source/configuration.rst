...
message objects based on the results of the HTTP interactions.
...
- Fields["ResponseTime"] (float64): Clock time elapsed for HTTP request,
                                    in seconds.
- Fields["Protocol"] (string): HTTP protocol used for the request (e.g.
                               "HTTP/1.0")
...
- method (string)
    ...
    Defaults to "GET".
- headers
    Subsection defining headers for the request.
    By default the User-Agent header is set to "Heka"
- body (string)
    The request body (e.g. for an HTTP POST request).
